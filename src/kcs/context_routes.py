...
from .engine import EngineError, OpenViking, document_uri, memory_uri, space_uri, subject_uri
...
def context(body: ContextInput, request: Request):
    ...
    with database.sessions.begin() as db:
        ...
        roots = [root, *[space_uri(auth.tenant_id, s) for s in spaces]]
    ...
    try:
        with factory() if factory else OpenViking(request.app.state.settings) as engine:
            hits = engine.find(roots if spaces else root, body.query, min(100, body.limit * 5))
    ...
    with database.sessions.begin() as db:
        ...
        for uri, _ in hits:
            for identity in spaces:
                prefix = space_uri(auth.tenant_id, identity) + "/"
